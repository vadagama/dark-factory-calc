...
- **AC-2** For every label other than `%`, the number of keys carrying that label is the same as it
  was before the change, and no pre-change label is missing or renamed.
  Check: `after_labels − before_labels == { "%" }` and `before_labels − after_labels == ∅` on the
  label lists, where `before_labels` is the keypad label list recorded from the pre-change build.
...
- **AC-4** No pre-change key changes behaviour: for every key sequence in the baseline regression
  suite enumerated in `scenario:example-product:calculator:existing-keys-regression` that does not
  press `%`, the display after the sequence is character-identical to the display the pre-change build
  produces for the same sequence.
